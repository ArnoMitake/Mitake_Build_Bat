...
set /p continue=是否要執行 Mvn package? (Y/N)
if /i "%continue%"=="Y" (
	call mvn package
	echo "Mvn package 執行完畢"
)
...
rem 啟用延遲擴張 防止變數改變 or 遺失 
setlocal enabledelayedexpansion

rem git 起始日期 格式: yyyy-MM-dd
set FROM_DATE=2023-05-29

rem git 至今日期 格式: yyyy-MM-dd
set TO_DATE=2023-05-30

rem 存放檔案位置
rem set Final_Target_File_Address="\\File-server\share\企業簡訊事業群\簡訊研發部\公用資料夾\【中心端調整】\20230526_測試用"
